...
trainer = Blueprint(
    ...
    static_url_path="/trainer/static",
    url_prefix="",
)


# trainer interface
@trainer.route("/trainer/myProfile")
def trainer1():
...
# trainer login
@trainer.route("/trainer", methods=["POST", "GET"])
def trainerLogin():
...
# trainer update
@trainer.route("/trainerUpdate", methods=["POST"])
def trainerUpdate():
...
@trainer.route("/trainerPWReset", methods=["POST"])
...
@trainer.route("/trainer/myTrainee")
def myTrainee():
    if "username" in session:
        username = session["username"]
        cur = getCursor()
        cur.execute(
            "SELECT Trainer.TrainerID FROM Trainer where Trainer.Email=%s", (username,)
        )
        ...
        cur.execute(
            "SELECT Member.MemberID, Member.FirstName, Member.LastName, Member.DateOfBirth, Member.MemberNotes, Booking.ClassID, Timetable.TrainerID, Timetable.ClassCode, Timetable.ClassDate \
        FROM Member \
        JOIN Booking ON Member.MemberID=Booking.MemberID \
        JOIN Timetable ON Booking.ClassID=Timetable.ClassID \
        WHERE (Timetable.ClassCode=1 AND Timetable.TrainerID=%s AND Timetable.ClassDate >=CURDATE()) ORDER BY Member.LastName, Member.FirstName, Member.MemberID, Timetable.ClassDate;",
            (TrainerID,),
        )
        ...
        return render_template(
            "myTrainee.html", TraineesDetails=TraineesDetails, username=username
        )
    else:
        return redirect(url_for("trainerLogin"))

...
def classes():
    ...
    result1 = db_manager.execute_query("SELECT ClassID FROM Booking WHERE MemberID = %s",(userId,))
    currentUserClassInfo=result1['result']
    BookedClassID=[ str(x[0]) for x in currentUserClassInfo]
    #get expired class info
    result2 = db_manager.execute_query("SELECT * FROM Timetable WHERE CONCAT(ClassDate, ' ', StartTime) < %s AND WEEKOFYEAR(ClassDate) = %s",(formatted_now_nz,WeekNum))
    expiredClassInfo=result2['result']
    ExpireClassID=[ str(x[0]) for x in expiredClassInfo]
    #get date
    result3 = db_manager.execute_query("""SELECT DISTINCT
                    'Date' AS StartTime,
    ...
    dbcols=result4['dbcols']
    dbresult=result4['result']
    listdb=[]
    listlayer=[]
    listclass=[]
    for x in dbresult:
...
def addClasse():
    if "username" in session:
        username = session["username"]
        #Get MemberID
        result1 = db_manager.execute_query("SELECT * FROM Member where Member.Email=%s",(username,))
        ...
        if memberStatus!='Inactive':
            ...
            if WaitForProcess=='1':
                ...
                result3 = db_manager.execute_query("""select ClassID from Booking
                ...
                if dbresultValidation==[]:
                    ...
                    db_manager.execute_query("insert into Booking (MemberID,ClassID,IsPaid,BookingStatus) values(%s,%s,'0','Current')",(MemberID,ClassID),commit=True)
                    ...
                    #NOTICE INSERT
                    result7 = db_manager.execute_query("SELECT ClassCode from Timetable where ClassID=%s",(ClassID,))
                    dbClassCode=result7['result']
                    now = datetime.now()
                    if dbClassCode[0][0]!=1:
                        db_manager.execute_query("INSERT into Notice(MemberID,NoticeDate,NoticeSubject,Content) VALUES( %s, %s, 'ClassBooked', 'You have booked a class successfully!')",(MemberID,now),commit=True)
...
def ptCalendar():
    if "username" in session:
        ...
        userID = ''
    #get booked class info
    currentUserClassInfo = db_manager.execute_query("SELECT ClassID FROM Booking WHERE MemberID = %s",(userID,))['result']
    BookedClassID=[ str(x[0]) for x in currentUserClassInfo]
    #get user input date info
    dateChosen=request.args.get("dateChosen","")
...
def ptsession():
    if "username" in session:
        userID = session["userID"]
        memberIDs = db_manager.execute_query("SELECT * FROM role WHERE role='member'")['result']
        memberIDList = []
        for memberID in memberIDs:
            memberIDList.append(memberID[0])
        if userID in memberIDList:
            username = session["username"]
            member_result = db_manager.execute_query("SELECT MemberStatus FROM Member WHERE Email=%s",(username,))['result']
            ...
            """
            timetableList = db_manager.execute_query(sql_timetable,(datetime.today(),))['result']
            ...
            existedSessionData = db_manager.execute_query(sql_existed_session,(datetime.today(),))['result']
            existedSessionList = []
        ...
        dayHelper = [1,2,3,4,5,6,7]
        weekDayList = [(1,'SUN'),(2,'MON'),(3,'TUE'),(4,'WED'),(5,'THU'),(6,'FRI'),(7,'SAT')]
        return render_template("ptSession.html", username="", trainerList=trainerList, timetableList=timetableList, weekDayList=weekDayList, dayHelper=dayHelper)
# book session function
@sessions.route("/bookSession",methods=["POST"])
def bookSession():
    username = session["username"]
    member = db_manager.execute_query("SELECT * FROM Member where Member.Email=%s",(username,))['result'][0]
    memberID = member[0]
    classID=request.form['classID']
    ...
    if len(bankaccount)==16  and len(bankcvc)>2 and isCardValid :
        ...
        sql_addBooking = """INSERT INTO Booking(MemberID,ClassID,IsPaid,BookingStatus)
                            VALUES(%s,%s,%s,%s) """
        db_manager.execute_query(sql_addBooking,(memberID,classID,1,"Current"),commit=True)
        # flash("You have booked a PT session successfully!")

        # Notice add
        dbClassCode=db_manager.execute_query("SELECT ClassCode from Timetable where ClassID=%s",(classID,))['result']
        now = datetime.now()
        if dbClassCode[0][0]==1:
            db_manager.execute_query("INSERT into Notice(MemberID,NoticeDate,NoticeSubject,Content) VALUES( %s, %s, 'Deduction', 'You have made a payment with $50')",(memberID,now),commit=True)
        #Notice add
...
def cancelClass():
    ...
    if time_delta.days < 0:
        ...
        if time_delta.days >7 or (time_delta.days <=7 and ClassCode!=1):
            ...
            dbClassCode=db_manager.execute_query("SELECT ClassCode from Timetable where ClassID=%s",(ClassID,))['result']
            if dbClassCode[0][0]!=1:
                db_manager.execute_query("INSERT into Notice(MemberID,NoticeDate,NoticeSubject,Content) VALUES( %s, %s,'CancelClass', 'You have cancel a class')",(MemberID,now),commit=True)
            else:
                db_manager.execute_query("INSERT into Notice(MemberID,NoticeDate,NoticeSubject,Content) VALUES( %s, %s,'Refund', 'You have cancelled a PT session and received a refund of $50')",(MemberID,now),commit=True)
            #NOTICE INSERT
            ...
            #NOTICE INSERT
            db_manager.execute_query("INSERT into Notice(MemberID,NoticeDate,NoticeSubject,Content) VALUES( %s, %s,'CancelClass', 'You have successfully canceled the PT session.Cancellation within seven days is non-refundable.')",(MemberID,now),commit=True)
            #NOTICE INSERT
            flash('You have successfully canceled the PT session, but as the cancellation was made within one week of the session, you will not receive a refund.!',"successCancelled")
            return redirect("/myBooking")